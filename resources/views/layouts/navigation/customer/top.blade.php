    ...
    <div
        ...
        <div class="flex items-center justify-between w-full md:w-auto shrink-0">
            ...
            <div class="flex items-center gap-3 shrink-0">
                <div
                    class="relative flex items-center justify-center w-11 h-11 rounded-2xl bg-gradient-to-br from-primary to-secondary shadow-lg shadow-(--color-primary)/20">
                    <i class="ti ti-bowl-spoon text-white text-2xl"></i>
                    <div class="absolute -top-1 -right-1 w-3 h-3 rounded-full bg-tertiary border-2 border-white"></div>
                </div>

                <div class="flex flex-col leading-none">
                    <span class="text-2xl font-black tracking-tight text-primary">Foody</span>
                    <span class="text-[10px] font-medium tracking-tighter text-gray-500">E-Kantin SMKN 1 Cirebon</span>
                </div>
            </div>

            <div class="relative group cursor-pointer md:hidden">
                <div class="w-10 h-10 rounded-xl p-0.5 bg-gradient-to-tr from-amber-500 to-amber-300 shadow-sm">
                    <div
                ...
                <div
                    ...
                    <div class="px-4 py-3 bg-slate-50 border-b border-slate-100">
                        <p class="text-[10px] text-slate-400 uppercase tracking-wider font-bold">Siswa / Guru</p>
                        <p class="text-sm font-bold text-slate-800 truncate">{{ Auth::user()->name }}</p>
                    </div>
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit"
                            class="w-full flex items-center gap-2 px-4 py-3 text-sm text-red-600 hover:bg-red-50 font-bold text-left transition-colors">
        ...
        <div class="relative group cursor-pointer hidden md:block shrink-0">
            <div class="flex items-center gap-3 pl-4 border-l border-slate-200/80">
                <div class="flex-col text-right leading-none hidden sm:flex">
                    <span
                        class="text-sm font-bold text-slate-700 group-hover:text-amber-600 transition-colors">{{ Auth::user()->name }}</span>
                    <span
                        class="text-[9px] font-bold text-slate-400 tracking-wider uppercase mt-0.5">{{ Auth::user()->role }}</span>
                </div>
                <div
                    class="w-10 h-10 md:w-11 md:h-11 rounded-xl p-0.5 bg-gradient-to-tr from-amber-500 to-amber-300 shadow-sm transition-transform duration-200 group-hover:scale-105">
                    <div
            ...
            <div
                ...
                <div class="px-4 py-3 bg-slate-50/80 border-b border-slate-100">
                    <p class="text-[10px] text-slate-400 uppercase tracking-wider font-bold">No. Identitas</p>
                    <p class="text-sm font-bold text-slate-800 truncate">{{ Auth::user()->id_number ?? '-' }}</p>
                </div>
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit"
